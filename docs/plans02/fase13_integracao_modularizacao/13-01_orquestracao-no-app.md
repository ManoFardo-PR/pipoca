...
>
> 🔄 **RECONCILIADO · 2026-07-12 · o portão LÊ o realizado (após A2)** — o STATUS acima descrevia o entregue: a T5 lia a PRÉVIA determinística (v3 "Joana") e a LLM só disparava no COMMIT, DEPOIS da leitura — o oposto da decisão fixada na linha 50 ("o texto LIDO no portão é o realizado"). A análise A2 registrou a divergência código↔doc. Corrigido (P2): a realização dispara na ENTRADA do portão (`prepararLeituraPortao`, sobre a composição pós-move SIMULADA); a T5 abre em "preparando" (espera com o teto de 8s) e LÊ o texto REALIZADO quando chega — estourou/falhou ⇒ prévia A+ crua (fallback visível, origem sinalizada). O resultado é memoizado ANTES da leitura e o commit o reusa ⇒ **salvo === lido**. A prévia da T4 (por movimento) segue determinística (zero LLM por movimento; e2e continua provando). A "hipótese prévia↔texto-final" da nota de jardim (linha 50) foi assim OBSERVADA e RESOLVIDA. Linhagem nunca se apaga — o texto original permanece.
>
...
> 🔄 **Reconciliado com o código (2026-07-12, após A2 — ver nota no topo):** a segunda metade da decisão ("o texto LIDO no portão é o realizado") não estava implementada — o código lia a prévia crua e disparava a LLM só no commit. A tensão entre "LLM só no COMMIT" e "o texto lido é o realizado" (as duas na mesma linha) foi resolvida a favor da leitura do realizado, SEM quebrar o orçamento de latência: a realização dispara UMA vez por rodada na ENTRADA do portão (não por movimento), a espera acontece DURANTE a transição T4→T5 com o teto de 8s, e o cru é a reserva visível. A prévia por MOVIMENTO da T4 segue determinística (zero LLM). "Prévia" volta a significar só a pré-visualização em T4.
